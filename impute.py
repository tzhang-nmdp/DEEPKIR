...
def impute(args):
    ...
    for batch_i in range(num_batch):
        ...
        for g in model_config:
            ...
            for digit in digit_list:
                logger.log('Imputing {} at {} level.'.format(', '.join(kir_list), digit))
                # Count kir alleles
                allele_cnts = {}
                all_one_allele = True
                skip_kirs = []
                ...
                if all_one_allele:
                    ...
                    for kir in kir_list:
                        result_phased.loc[kir_info[kir][digit], sample_fam_batch.iid] = np.full((1, 2*num_sample_batch), 'P')
                        result_dosage.loc[kir_info[kir][digit], sample_fam_batch.iid] = np.full((1, num_sample_batch), 2.000)
                        if mc_dropout:
                            result_entropy.loc[kir_info[kir][digit], sample_fam_batch.iid] = 0
                    continue
                ...
                for kir in kir_list:
                    if not allele_cnts[kir] == 1:
                        model[kir] = EachNet(model_config[g], allele_cnts[kir])
                        model[kir].load_state_dict(torch.load(os.path.join(model_dir, '{}_{}_{}_model.pickle'.format(g, digit, kir))))
                for m in model:
                ...
                for kir in kir_list:
                    if allele_cnts[kir] == 1:
                        ...
                        imputed, mask_fc = model[kir](shared, None)
                        imputed = np.array(np.exp(imputed.detach()).T)
                        phased = np.full((allele_cnts[kir], 2*num_sample_batch), 'A')
                    ...
                    result_phased.loc[kir_info[kir][digit], sample_fam_batch.iid] = phased
                    result_dosage.loc[kir_info[kir][digit], sample_fam_batch.iid] = dosage
                ...
                if mc_dropout:
                    ...
                    for kir in kir_list:
                        if not allele_cnts[kir] == 1:
                            prediction_stack[kir] = np.zeros((2*num_sample, allele_cnts[kir]))
                    for m in model:
                        model[m].train()
                    for t in range(num_sampling):
                        ...
                        for kir in kir_list:
                            if not allele_cnts[kir] == 1:
                                ...
                                for i in range(2*num_sample_batch):
                                    prediction_stack[kir][i, np.argmax(imputed[:, i])] += 1
                    # Calculate entropy for each allele of each sample
                    for kir in kir_list:
                        if not allele_cnts[kir] == 1:
                            for i in range(allele_cnts[kir]):
                                tmp = np.zeros(2*num_sample_batch)
                                for j in range(2*num_sample_batch):
                                    tmp[j] = entropy([prediction_stack[kir][j][i], num_sampling-prediction_stack[kir][j][i]])
                                result_entropy.loc[kir_info[kir][digit][i], sample_fam_batch.iid] = tmp
                        else:
                            result_entropy.loc[kir_info[kir][digit], sample_fam_batch.iid] = 0
            result_phased.to_csv('{}.batch{}.deepkir.phased'.format(out_prefix, batch_i+1), header=True, index=True, sep='\t')
            result_dosage.astype(np.float64).round(3).to_csv('{}.batch{}.deepkir.dosage'.format(out_prefix, batch_i+1), header=True, index=True, sep='\t')
            if mc_dropout:
                result_entropy.to_csv('{}.batch{}.deepkir.entropy'.format(out_prefix, batch_i+1), header=True, index=True, sep='\t')

    # Merge result files
    result_phased = pd.DataFrame()
    result_dosage = pd.DataFrame([['P', 'A'] for _ in range(len(result_index))], index=result_index)
    for batch_i in range(num_batch):
        result_phased_batch = pd.read_csv('{}.batch{}.deepkir.phased'.format(out_prefix, batch_i+1), index_col=0, sep='\t')
        result_dosage_batch = pd.read_csv('{}.batch{}.deepkir.dosage'.format(out_prefix, batch_i+1), index_col=0, sep='\t')
    ...
    result_phased.to_csv('{}.deepkir.phased'.format(out_prefix), header=False, index=True, sep='\t')
    result_dosage.iloc[:, 2:] = result_dosage.iloc[:, 2:].astype(np.float64).round(3)
    result_dosage.to_csv('{}.deepkir.dosage'.format(out_prefix), header=False, index=True, sep='\t')
    for batch_i in range(num_batch):
        os.remove('{}.batch{}.deepkir.phased'.format(out_prefix, batch_i+1))
        os.remove('{}.batch{}.deepkir.dosage'.format(out_prefix, batch_i+1))
    if mc_dropout:
        result_entropy = pd.DataFrame()
        for batch_i in range(num_batch):
            result_entropy_batch = pd.read_csv('{}.batch{}.deepkir.entropy'.format(out_prefix, batch_i+1), index_col=0, sep='\t')
            result_entropy = pd.concat((result_entropy, result_entropy_batch), axis=1)
        result_entropy.to_csv('{}.deepkir.entropy'.format(out_prefix), header=False, index=True, sep='\t')
        for batch_i in range(num_batch):
            os.remove('{}.batch{}.deepkir.entropy'.format(out_prefix, batch_i+1))

    logger.log('The processes have been finished at {}.'.format(time.ctime()))


def main():
    parser = argparse.ArgumentParser(description='Perform KIR imputation with a trained model.')
